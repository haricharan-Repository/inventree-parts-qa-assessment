...
| PDV-17 | Revision selector shown when revisions exist | Part has 2 revisions (Rev A, Rev B) | 1. Open Rev A's detail page → Part Details tab. | **Verified against a live instance**: there is no separate "Revisions" tab — a "Select Part Revision" dropdown widget is rendered inside the *Part Details* tab's own content; selecting Rev B navigates to Rev B's detail page. | P1 | Positive |
| PDV-18 | Revision selector hidden for a part with no revisions | Part has no `revision_of` links | 1. Open part detail → Part Details tab. | No "Select Part Revision" widget displayed. | P3 | Negative |
